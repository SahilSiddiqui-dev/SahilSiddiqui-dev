<!-- ================= HEADER ================= --><h1 align="center">Hi 👋, I'm Mohd Sahil Siddiqui</h1>
<h3 align="center">🚀 Frontend Developer in Progress | React Learner</h3><p align="center">
  <img src="https://readme-typing-svg.herokuapp.com?size=28&duration=3500&color=00F7FF&center=true&vCenter=true&width=600&lines=Building+Projects+🚀;Learning+React+%26+Frontend;Improving+Everyday+Through+Code" />
</p>---

<!-- ================= ABOUT ================= -->💫 About Me

- 🚀 Currently working on React JS
- 🌱 Learning frontend fundamentals deeply
- 🛠️ Built multiple demo projects to practice concepts
- 🤝 Open to collaborating on beginner-friendly React projects
- 📚 Focused on strong fundamentals — no shortcut learning
- ⚡ I grow by building → breaking → fixing → repeating

«I’m still early in my dev journey — but consistent in progress.»

---

<!-- ================= TECH STACK ================= -->🛠️ Tech Stack

👨‍💻 Frontend

<p>
  <img src="https://skillicons.dev/icons?i=html,css,js,react,tailwind" />
</p>⚙️ Tools

<p>
  <img src="https://skillicons.dev/icons?i=git,github,vscode,vercel,netlify" />
</p>---

<!-- ================= PROJECTS ================= -->🚀 Projects Showcase

🌐 Landing Page

Responsive website built using HTML, CSS & JavaScript.

🔗 Live Demo: sahilsiddiqui.site
🔗 Repository: [Click Here To Open Repo](https://github.com/SahilSiddiqui-dev/My-portfolio](https://github.com/SahilSiddiqui-dev/Demo-Tour-Booking-Website)

---

🛒 Checkout Page (React + Next.js)

Component-based checkout UI with state handling.

🔗 Live Demo: Add Link Here
🔗 Repository: [Click Here To Open Repo](https://github.com/SahilSiddiqui-dev/Check-Out-Page)

---


📌 More Projects Coming Soon...

Currently building more real-world React applications.

---

<!-- ================= STATS ================= -->📊 GitHub Stats
![](https://github-readme-stats.vercel.app/api?username=SahilSiddiqui-dev&theme=dark&hide_border=false&include_all_commits=false&count_private=false)<br/>
![](https://nirzak-streak-stats.vercel.app/?user=SahilSiddiqui-dev&theme=dark&hide_border=false)<br/>
![](https://github-readme-stats.vercel.app/api/top-langs/?username=SahilSiddiqui-dev&theme=dark&hide_border=false&include_all_commits=false&count_private=false&layout=compact)
<!-- ================= CONTACT ================= -->🌐 Connect With Me

<p>
  <a href="mailto:sahilsiddiqui2424@gmail.com">
    <img src="https://img.shields.io/badge/Email-D14836?style=for-the-badge&logo=gmail&logoColor=white"/>
  </a>
</p>---

<!-- ================= GOALS ================= -->🧭 2026 Goals

- ⚛️ Master React ecosystem
- 🔗 Work with APIs
- 🔐 Learn authentication systems
- ☁️ Deploy full-stack apps
- 🧠 Improve system design basics

---

<p align="center">
  ⭐ Consistency &gt; Motivation
</p>
